...
var transporter = nodemailer.createTransport({
    service: 'gmail',
    auth:{
        user: '30agrawalvinayak@gmail.com',
        // pass: 'Hostel@2022'
        pass: 'RadheRadhe'
    }
});

// call auto_esc() written below in query quotes
app.get("/",function(req,res){
    client.query(``,(err,res2)=>{
        if(err){
            console.log(err);
            res.send(err);
        }
...
app.get("/view_notice",function(req,res){
    if (req.session.gid ==''){
        ...
        client.query(`select * from notice where owner = 'admin99' or owner = (select distinct(warden_ref_id) from hostel where hostel_id=(select hostel_ref_id from student where student_id='${req.session.gid}'));`,function(err,res2){
        if(err){
...
const client = new Client({
    ...
    port:5432,
    password:"Ind2gether",
    database:"HostelComplaintSystemDB"
})
client.connect();
                ...
                if (err) {
                    ...
                    client.query(`select * from student where student_id = (select student_ref_id from complaints where complaint_id='${id}')`, function (err3, res3) {
                        if (err3) {
                            ...
                            var mailOptions = {
                                from: '30agrawalvinayak@gmail.com',
